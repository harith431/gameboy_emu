...
std::vector<uint8_t> rom_data = {0x00 };

uint16_t rom_size = static_cast<uint16_t>(rom_data.size());
...
// ---------------- ADC (Add with Carry) ----------------
   ...
   if (mnemonic == "OR") {
       if (operand1 == "(HL)") {
           ...
           uint16_t x1 = cpu.A | resolve_register(operand1);
           cpu.A = x1 & 0xff;
           cpu.F = getFlagBytes(flags);
           cpu.clock_cycles += cycles[0];
       }
   ...
   if (mnemonic == "CP") {
    ...
    // ---------------- RET (Return from Subroutine) ----------------
        ...
        uint8_t hi = memory.read(cpu.STACK_P++);
        cpu.PC = (hi << 8) | lo;
...
// ---------------- LD (C), A ----------------
    ...
    int main() {
        ...
        cpu.F = 0x80;
            ...
            printf("inst_lenght = %d ", inst_length);
            printf("cycles = %d", cpu.clock_cycles);
            if (opcode == 0xCB) {
